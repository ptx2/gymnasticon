 ...
 */
export class EchelonBikeClient3 extends EventEmitter {
  /**
  ...
  async connect() {
    ...
    // some embedded BLE stacks don't like noble-hci's use of
    // Read By Type in subscribeAsync() so use Find Info instead
    ...
    // other init packets (unsure of meaning?)
    const INITDATA1_PKT = Buffer.from([0xf0, 0xa1, 0x00, 0x91]);
    const INITDATA2_PKT = Buffer.from([0xf0, 0xa3, 0x00, 0x93]);
    await this.rx.writeAsync(INITDATA1_PKT, false);
    await this.rx.writeAsync(INITDATA1_PKT, false);
    await this.rx.writeAsync(INITDATA1_PKT, false);
    await this.rx.writeAsync(INITDATA1_PKT, false);
    await this.rx.writeAsync(INITDATA2_PKT, false);
    await this.rx.writeAsync(INITDATA1_PKT, false);
